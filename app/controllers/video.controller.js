...
exports.createComment = (req, res, next) => {
  ...
  const post = new Comment({
      ...
      //répertoire images
      videoUrl: req.file ? `${req.protocol}://${req.get('host')}/images/${req.file.filename}` : '',
...
Comment.update(req.body, {
  ...
  .then(data => {
    res.send(data)
    if(data.videoUrl){ 
    const videoUrl = data.videoUrl.split('/images/')[1];
    fs.unlink(`images/${videoUrl}`, (err) => {
      // if (err) throw err;
      console.log(`images/${videoUrl} was deleted`);
      })
    Comment.update({
      videoUrl: `${req.protocol}://${req.get("host")}/images/${
      req.file.filename}`
      },
      { where: {id : id },
      })
